...
#| eval: false
library(tidycensus)
census_api_key("4fdc88c73f238cce1df0ae9fe0dbe48f2c513849", overwrite = FALSE, install = TRUE)
```
...
census_counties <- census_counties |> mutate(
  "black_per" = (value/summary_value)*100
)

census_tracts <- census_tracts |> mutate(
  "black_per" = (value/summary_value)*100)
##TA feedback: This looks awesome! +2 
...
pal_bin_census_county_perc <- colorBin(palette="Blues", 
                    domain = census_counties$black_per,
                    bins = 5)
...
la_map %>%
  addPolygons(data = census_counties, 
              weight = 1,
              color = "black",
              fillColor = ~pal_bin_census_county_perc(black_per),
              fillOpacity = 0.5,) %>%
...
pal_bin_census_tracts_perc <- colorBin(palette="Blues", 
                    domain = census_tracts$black_per,
                    bins = 5)
...
la_map %>%
  ...
  addPolygons(data = census_tracts, 
              weight = 1,
              color = "black",
              fillColor = ~pal_bin_census_tracts_perc(black_per),
              fillOpacity = 0.5,) %>%
  addLegend(data = census_tracts,
            title = "% Black Pop - Tract", 
            pal = pal_bin_census_tracts_perc, 
            values = ~black_per)
##TA Feedback: This looks beautiful! +2
...
Answer:
This is an example of ecological fallacy because aggregated data (census tract) is showing how I high percentage of black residents line near this area. However, one would be incorrect to use this information from aggregated data and claim everyone living along the Mississippi River are black just because of what this aggregated data shows.
:::
...
##TA Feedback: This looks fabolous! +4
...
la_map %>%
  ...
  hideGroup("County")
##TA Feedback: This looks marvelous! +4
##Total for Q1-Q10: 27
```
...
Answer:
One way I think the modifiable aerial unit problem may be at play is because of how not uniform the boundaries for the tracts are. Yes, there are places where there are a lot of high-polluting industrial facilities with lower percentages of black populations, but that is just because of how the lines are drawn. There are definitely ways these boundaries can be changed where it then looks like there are a lot of high-polluting industrial facilities with high percentages of black populations. We need to consider the aerial unit problem when looking at pollution data and really ask why the data is aggregated and broken up in that way, is it to push what the creator of the graph wants you to think?
##TA Feedback: This is a great answer! +3
##Total for Q1-Q11: 30 pts
...
##TA Grading: Final Score: 30/30 or 100%
