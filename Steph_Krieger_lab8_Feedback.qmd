...
open_payments_dates_cleaned <- open_payments_original |> mutate(across(contains("Date"), mdy))
...
open_payments <- open_payments_dates_cleaned |>
  mutate(across(contains("Covered_Recipient"), str_to_title)) |>
...
```{r}
# Uncomment below and write data wrangling code 305074.90 829811.8
...
top_10_doctors_ids <- top_10_doctors |> select(Covered_Recipient_NPI) |> pull()
...
map_chr(top_10_doctors_ids, get_doctor_name)
...
map(top_10_doctors_ids, calculate_payment_type_amts) |> set_names(top_10_doctors_names)

# Iterate calculate_payment_type_amts over top_10_doctors_ids and set names to top_10_doctors_names
## TA Feedback: These look fantastic! +3
...
##TA Feedback: These look wonderful! +3
##Total for Q1-Q9: 27 pts
```
...
calculate_manufacturer_payments <- function(doctor_id) {
   open_payments <- open_payments |>
     mutate(Total_Amount_of_Payment_USDollars = as.numeric(Total_Amount_of_Payment_USDollars))|>
    filter(Covered_Recipient_NPI == doctor_id) |> 
...
manufacturer_payments_df <- map_df(top_10_doctors_ids, calculate_manufacturer_payments)

# Plot resulting data frame here

ggplot(manufacturer_payments_df, aes(
  x = Applicable_Manufacturer_or_Applicable_GPO_Making_Payment_Name,
  y = total_payments,
  fill = Form_of_Payment_or_Transfer_of_Value)) +
  geom_col() + 
  coord_flip() +
  facet_wrap(vars(Covered_Recipient_NPI), scales = "free", nrow = 5) +
  labs(
    title = "Total Amount of Medical Drug/Device Payments to 10 MA-Based Doctors, 2024",
    x = "Total Amount of Payment ($)",
    y = "Name of manufacturer",
    fill = "Form of Payment?"
  ) 
##TA Feedback: This looks wondrous! +3
##Total for Q1-Q10: 30 pts
```
...
drugs_and_devices <- function(doctor_id, doctor_name){
      open_payments |>
    filter(Covered_Recipient_NPI == doctor_id,
           year(Date_of_Payment) == 2024) |>   mutate(Total_Amount_of_Payment_USDollars = as.numeric(Total_Amount_of_Payment_USDollars)) |>
        group_by(Name_of_Drug_or_Biological_or_Device_or_Medical_Supply_1,
             Nature_of_Payment_or_Transfer_of_Value) |>
    summarize(total_amount = sum(Total_Amount_of_Payment_USDollars, 
                                na.rm = TRUE),
              .groups = "drop") |>
       ggplot(aes(x = reorder(Name_of_Drug_or_Biological_or_Device_or_Medical_Supply_1, 
                           total_amount),
               y = total_amount,
               fill = Nature_of_Payment_or_Transfer_of_Value)) +
    
    geom_col() +
    coord_flip() +
    
    labs(
      title = paste("Payments by Drug/Device for Doctor", doctor_id, "(2024)"),
      x = "Drug / Biological Device",
      y = "Total Payment (USD)",
      fill = "Nature of Payment"
    ) +
...
map(top_10_doctors_ids, drugs_and_devices)
##TA Feedback: Nice job on this one! +3
##Total for Q1-Q11: 33 pts
```
...
:::
##TA Grading: Final Score 33/30 or 110%
