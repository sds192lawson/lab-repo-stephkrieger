...
open_payments_dates_cleaned <- open_payments_original |>
  mutate(Date_of_Payment = mdy(Date_of_Payment), Payment_Publication_Date = mdy(Payment_Publication_Date))

#| echo: false
#| document: solutions
open_payments_dates_cleaned <-
  open_payments_original |>
...
open_payments <- open_payments_dates_cleaned |>
  ...
  mutate(Covered_Recipient_Full_Name = paste(Covered_Recipient_First_Name, Covered_Recipient_Last_Name, sep = " "))
...
get_doctor_name <- function(doctor_id){
  open_payments |>
    filter(Covered_Recipient_First_Name == doctor_id)
...
top_10_doctors_names <- open_payments |>
  map_chr(get_doctor_name)
##TA Feedback: This look wondrous! +3
...
##TA Feedback: remember to answer this question! +0
...
:::
##TA Grading: Final Score: 21/30 or 70%
