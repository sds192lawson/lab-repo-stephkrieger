...
# the unit of observations is one singular stop and the variables describe the precinct, age and race of suspect, if they had a weapon or not, if arrested or not, and if the suspect frisked.
...
#605328 did not result in an arrest. 88.28%.
...
#346226 individuals aged 14-24. 50.49% of stops. These don't match the values on the NYCLU website because I didn't take out the N/A which are reported as 999, skewing the numbers.
...
sqf_2011 |>
  ...
  arrange(desc(stops)) 
##TA feedback: This looks tremendous! +3
...
sqf_2011 |>
  ...
  filter(race_cat == c("WHITE-HISPANIC", "BLACK-HISPANIC")) |> 
  #Calculate number of observations
...
sqf_2011 |>
  ...
  group_by(race_cat) |> 
  summarise(stops = n(),
            percent_stops = n() / stops_race * 100,
            percent_frisked = sum(frisked) / stops_race *100,
            percent_wpnfound = sum(wpnfound) / stops_race *100,
            percent_arrestsumm = sum(arrestsumm) / stops_race *100
            ) |>
  arrange(desc(stops)) 
##TA Feedback: This looks terrific just remember to add na.rm=TRUE to each part of your summarise next time. +2.5
...
```{r, eval = FALSE}
#Based on the code above I notice how there is a disproportionate amount of stops depending on the race of the suspect. Black and White-Hispanic suspects are stopped way more than White, Black-Hispanic, Asian/Pacific Islander, or American Indian/Alaskan Native suspects. This information may lead to calling the 2011 stop and frisk activity unconsitutional because it seems to be targeting black and white-hispanic individuals.
##TA Feedback: Great answer! +3
