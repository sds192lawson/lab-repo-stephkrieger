...
pit_pivoted <- pit |> pivot_longer(cols = -State,
                                   names_to = "Measure",
                                   values_to = "Value")
##TA Feedback: This looks great, just remember that next time you can remove the state column by just writing 2:117 at the start of your pivot_longer() instead. -1 for incorrect arguement
##Total for Q1: 2 pts
```
...
##TA Feedback: This looks awesome! +4
...
pit_by_gender <- pit_str_cleaned |> filter(Demographic %in% c("Female", "Male", "Transgender"))
pit_by_race <- pit_str_cleaned |> filter(Demographic %in% c("Black or African American", "Asian", "American Indian or Alaska Native", "Native Hawaiian or Other Pacific Islander", "White", "Multiple Races"))
##TA Feedback: This looks extraordinary! +4
...
race_final <- race_separated |> pivot_wider(names_from = "Measure", 
                   values_from = "Value", 
                   names_repair = make.names)
##TA Feedback: This looks terrific! you should not need the names_repair argument at the end there though. +4
...
pit_gender <- pit_by_gender |> inner_join(gender_final, by = c("StateName" = "NAME", "Demographic" = "variable", "Year" = "Year"))
pit_race <- pit_by_race |> inner_join(race_final, by = c("StateName" = "NAME", "Demographic" = "variable", "Year" = "Year"))
##TA Feedback: This looks wondrous! +4
...
pit_gender_rates <- pit_gender |> mutate(homeless_rate = Value/estimate * 10000)

pit_race_rates <- pit_race |> mutate(homeless_rate = Value/estimate * 10000)
